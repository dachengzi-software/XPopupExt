...
public class CityPickerPopup extends BottomPopupView {
    ...
    private void initJsonData() {//解析数据
        ...
        if (jsonBeanList == null || jsonBeanList.size() == 0) {
            ...
            jsonBeanList = parseData(JsonData);//用Gson 转成实体
        }
        for (int i = 0; i < jsonBeanList.size(); i++) {//遍历省份
            options1Items.add(jsonBeanList.get(i).getName());
            ...
            for (int c = 0; c < jsonBeanList.get(i).getCityList().size(); c++) {//遍历该省份的所有城市
                ...
                city_AreaList.addAll(jsonBeanList.get(i).getCityList().get(c).getArea());
